...
- [~] **Preview-state guardrail — DEFERRED (2026-07-04, tracked).** ⏸️ Cloudflare Access on preview URLs **skipped for now**: the app is scaffold-only, so there is **no claim-status data to leak yet**, and this guardrail is scoped "before any claim-status code exists." **🔒 HARD RE-ADD TRIGGER:** before the first commit that adds claim/reveal/organizer-view code, gate preview URLs behind **Cloudflare Access** *or* point previews at a separate Supabase project. This is the CLAUDE.md organizer-blindness rule — non-negotiable once claim code exists. (Preview-branch protection is a later-course topic per the user.)
  - **(2026-09-11, F-02) Still open.** F-02 landed the claim/items/claims schema, RLS and RPCs — no UI. Previews still hit **production** Supabase, so the trigger has not fired yet, but it is now close: it must fire before S-02 or S-03 ship any claim/reveal/organizer-view UI. Deferred by the user to the staging-environment lesson; not resolved by F-02.
- [x] **Validate** — ✅ end-to-end via **PR #6** (`m1l5-followup`: `.nvmrc` + homepage tweak, merge commit). Branch push auto-created a **preview** version `365fc3f6` (`versions upload`); merge to `main` triggered Workers Builds → `wrangler deploy` → production version **`d55e292e`** (deployed 2026-07-04 08:05). Live URL now serves "Welcome to Gift I Need!" (body), health 200. **Node 24 from `.nvmrc` built cleanly** in Workers Builds. *(Self-merge by the agent was blocked by the auto-mode classifier — the human merged, preserving review.)*
- [x] **Observability check** — ✅ `wrangler tail --format json` against production captured 6 live events, all `outcome: "ok"`, served by version `d55e292e`, across `/` + `/api/health`. Same logs/metrics visible in the Worker → **Observability** tab (enabled via `wrangler.jsonc` Phase 1).
