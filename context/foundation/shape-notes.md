...
created: 2026-06-07
updated: 2026-06-07  # finalised
checkpoint:
  current_phase: 8
  phases_completed: [1, 2, 3, 4, 5, 6, 7]
  gray_areas_resolved:
    - topic: "Context type"
      decision: "Greenfield — fresh project, no existing application code"
    - topic: "MVP scope"
      decision: "Scoped to flows 1+2 only: event organizer creates/shares list, guests claim items, post-event confirmation. Flow 3 (personal idea-capture) is explicitly v2."
    - topic: "Auth model"
      decision: "Email + password or OAuth (Google/Apple) for all users; no anonymous claiming — account required to claim a gift"
    - topic: "Guest vs owner distinction"
      decision: "No permanent guest role — a guest who claims a gift is the same account type as a list owner; entry point differs, not the role"
    - topic: "Claim visibility"
      decision: "Guests see items as 'claimed' (no name shown); organizer cannot see claim status at all until post-event"
    - topic: "Timeline"
      decision: "4 weeks after-hours for v1 (flows 1+2 only)"
  frs_drafted: 14
...
- **Sign-up / sign-in**: email + password, or OAuth (Google or Apple).
...
- Once claimed, the item shows as "taken" to all other signed-in guests immediately
...
- **OAuth login (Google/Apple)**: email + password only for v1. OAuth is a v2 enhancement once the core flow is proven.
...
- **Target scale**: medium (dozens to a few hundred users)
- **Timeline budget**:
  - mvp_weeks: 4
  - hard_deadline: null
