...
## A cheap local "is it revealed" check is safe to be wrong — only the DB-side re-check is security-relevant

- **Context**: Any place a JS-side approximation of a DB-side time/state gate exists alongside a real DB re-check on the actual protected write/read path. GIN's concrete instance: `src/lib/lists/owned-events.ts:160-162`'s `revealOpen` (computed in JS from an already-fetched event row) vs. `private.reveal_open` re-derived live on every call inside `private.get_shared_items` (`supabase/migrations/20260911233636_single_reveal_gate_and_policy_subplan.sql:81`, `when e.owner_id = auth.uid() and not private.reveal_open(e.id) then null`).
- **Problem**: Two independent "is it revealed" computations exist by design — a cheap JS gate used only to skip a doomed RPC call (avoid fetching status that would be discarded), and an authoritative DB gate re-evaluated on every call. It's tempting to treat JS-side drift (stale fetch, clock skew, a DST edge case) as a claim-data leak risk, but it structurally isn't: the JS gate only decides *whether to call* `getSharedList`, never *what gets rendered* — the DB gate is what actually nulls the claim status, independent of what the JS side believed. `organizer-view.integration.test.ts` (`testing-organizer-blindness-claim-integrity` Phase 2) proved this against a real guest claim and the real RPC, not a mock.
- **Rule**: When a cheap local check exists purely to skip a call whose result would be discarded anyway, it is safe for that check to be wrong occasionally — don't "fix" it to be more precise, and don't treat its drift as a security bug. Only the authoritative re-check that actually gates the returned data is load-bearing; that's the one that needs security-relevant test coverage. Document explicitly, at the cheap check's definition site, which downstream check is the real one (already true of `owned-events.ts:152-159`'s comment).
- **Applies to**: plan, implement, impl-review
