...
**Addendum (impl-review, 2026-09-12 — plan correction)**: this assumption was wrong. Next.js 16 Server Actions do **not** automatically re-render the invoking Server Component on completion. The shipped implementation calls `refresh()` from `next/cache` before returning, which is what actually re-runs `getOwnedEvent` and gets the new item into `ItemList`. See `src/app/actions/events.ts`.
